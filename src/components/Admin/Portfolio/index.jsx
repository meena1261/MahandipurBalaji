...
const PortfolioTable = () => {
    ...
    columnHelper.accessor("slug", {
      ...
      cell: (info) => (
        <div className="flex gap-2">
          <Link href={`/admin/adminPortfolio/details?slug=${info.getValue()}`}>
            <button className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
              <Eye className="w-4 h-4 text-gray-600 dark:text-gray-400" />
            </button>
          </Link>
          <Link href={`/admin/adminPortfolio/addEdit?id=${info.getValue()}`}>
            <button className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/portfolio?id=${id}`, {
          method: "DELETE",
        });
      ...
      <div className="p-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          ...
          <div className="flex gap-4 w-full sm:w-auto">
            ...
            <Link href="/admin/adminPortfolio/addEdit">
